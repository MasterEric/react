...
function deleteHydratableInstance(returnFiber, instance) {
  ...
  childToDelete.flags = childToDelete.flags & StaticMask | Deletion; // This might seem like it belongs on progressedFirstDeletion. However,
...
function mountEffect(create, deps) {
  ...
  return mountEffectImpl(Update | Passive | PassiveStatic, Passive$1, create, deps);
}
...
function updateSuspensePrimaryChildren(current, workInProgress, primaryChildren, renderLanes) {
  ...
  if (currentFallbackChildFragment !== null) {
    ...
    currentFallbackChildFragment.flags = currentFallbackChildFragment.flags & StaticMask | Deletion;
...
function updateSuspenseFallbackChildren(current, workInProgress, primaryChildren, fallbackChildren, renderLanes) {
  ...
  workInProgress.child !== currentPrimaryChildFragment) {
    ...
    if (currentFallbackChildFragment !== null) {
      ...
      var firstPrimaryTreeEffect = currentFallbackChildFragment.nextEffect;
    ...
    if ((nextEffect.flags & ChildDeletion) !== NoFlags) {
      ...
      if (deletions !== null) {
        for (var i = 0; i < deletions.length; i++) {
          ...
          commitPassiveUnmountEffectsInsideOfDeletedTree_begin(fiberToDelete); // Now that passive effects have been processed, it's safe to detach lingering pointers.
...
function commitPassiveUnmountEffectsInsideOfDeletedTree_complete(deletedSubtreeRoot) {
  while (nextEffect !== null) {
    var fiber = nextEffect; // TODO: Check if fiber has a PassiveStatic flag
...
function commitRootImpl(root, renderPriorityLevel) {
    ...
    do {
      {
        ...
        if (hasCaughtError()) {
          ...
          var error = clearCaughtError();
          captureCommitPhaseError(nextEffect$1, error);
          nextEffect$1 = nextEffect$1.nextEffect;
        }
      }
    } while (nextEffect$1 !== null); // We no longer need to track the active instance fiber
    ...
    do {
      {
        ...
        if (hasCaughtError()) {
          ...
          captureCommitPhaseError(nextEffect$1, _error);
          nextEffect$1 = nextEffect$1.nextEffect;
        }
      }
    ...
    do {
      {
        ...
        if (hasCaughtError()) {
          ...
          captureCommitPhaseError(nextEffect$1, _error2);
          nextEffect$1 = nextEffect$1.nextEffect;
        }
      }
    } while (nextEffect$1 !== null);
  ...
  if (rootDoesHavePassiveEffects) {
    ...
    while (nextEffect$1 !== null) {
      var nextNextEffect = nextEffect$1.nextEffect;
      nextEffect$1.nextEffect = null;

      if (nextEffect$1.flags & ChildDeletion) {
        var deletions = nextEffect$1.deletions;
  ...
  if (remainingLanes !== NoLanes) {
    {
      if (spawnedWorkDuringRender !== null) {
        ...
        for (var _i = 0; _i < expirationTimes.length; _i++) {
          scheduleInteractions(root, expirationTimes[_i], root.memoizedInteractions);
        }
      }
...
function commitBeforeMutationEffects() {
  while (nextEffect$1 !== null) {
    ...
    if (!shouldFireAfterActiveInstanceBlur && focusedInstanceHandle !== null) {
      if ((nextEffect$1.flags & Deletion) !== NoFlags) {
        if (doesFiberContain(nextEffect$1, focusedInstanceHandle)) {
          shouldFireAfterActiveInstanceBlur = true;
          beforeActiveInstanceBlur(nextEffect$1);
        }
      } else {
        // TODO: Move this out of the hot path using a dedicated effect tag.
        if (nextEffect$1.tag === SuspenseComponent && isSuspenseBoundaryBeingHidden(current, nextEffect$1) && doesFiberContain(nextEffect$1, focusedInstanceHandle)) {
          shouldFireAfterActiveInstanceBlur = true;
          beforeActiveInstanceBlur(nextEffect$1);
        }
      }
  ...
  while (nextEffect$1 !== null) {
    ...
    var primaryFlags = flags & (Placement | Update | Deletion | Hydrating);
     ...
     switch (primaryFlags) {
      case Placement:
        {
          commitPlacement(nextEffect$1); // Clear the "placement" from effect tag so that we know that this is
          // inserted, before any life-cycles like componentDidMount gets called.
          // TODO: findDOMNode doesn't rely on this any more but isMounted does
          // and isMounted is deprecated anyway so we should be able to kill this.

          nextEffect$1.flags &= ~Placement;
          break;
        }
      ...
      case PlacementAndUpdate:
        {
          ...
          nextEffect$1.flags &= ~Placement; // Update
      ...
      case Hydrating:
        {
          nextEffect$1.flags &= ~Hydrating;
          break;
        }

      case HydratingAndUpdate:
        {
          nextEffect$1.flags &= ~Hydrating; // Update
      ...
      case Update:
        {
          var _current3 = nextEffect$1.alternate;
          commitWork(_current3, nextEffect$1);
          break;
        }
      ...
      case Deletion:
        {
          ...
          var deletedChild = nextEffect$1;
          var deletions = deletedChild.deletions;

          if (deletions !== null) {
            for (var i = 0; i < deletions.length; i++) {
              var deletion = deletions[i]; // Clear the deletion effect so that we don't delete this node more
              // than once.

              deletion.flags &= ~Deletion;
              deletion.deletions = null;
              commitDeletion(root, deletion);
            }
          }
...
function commitLayoutEffects(root, committedLanes) {
  ...
  while (nextEffect$1 !== null) {
    ...
    if (flags & (Update | Callback)) {
      var current = nextEffect$1.alternate;
      commitLifeCycles(root, current, nextEffect$1);
    }
...
function resetWorkInProgress(workInProgress, renderLanes) {
  ...
  workInProgress.flags &= StaticMask | Placement; // The effect list is no longer valid.
